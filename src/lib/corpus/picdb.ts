 ...
 * Lee bol_bible_refs / bol_bible_urls de la BD app (prefijo bol_, como el resto).
 */
...
export class Picdb {
  ...
  get_pics(bcv: (string | number)[]): (number | string)[] | null {
    const rows = this.db
      .prepare("SELECT booknumber,picture FROM bol_bible_refs WHERE book=? AND chapter=? AND verse=?")
  ...
  get_urls(bcv: (string | number)[]): { url: string; type: string }[] | null {
    const rows = this.db
      .prepare("SELECT url,type FROM bol_bible_urls WHERE book=? AND chapter=? AND verse=?")
