...
- [x] File manager + upload (Ctrl_file_manager, Ctrl_upload, valums) — route handlers multipart: métodos de escritura de `Mod_quizpath` (mkdir/rename/rmdir/delete_files/chown_files/fix_exerciseowner/insert_files/insertPassages) + `Mod_classdir` (update_classes_for_dir/remove_classes_for_dir) en `quizpath.ts`, cookie firmada `ol_fm` para copy/move (`src/lib/auth/fm-session.ts`), acciones en `src/app/actions/file-manager.ts` (guardas teacher/admin 1:1), páginas `/file_manager` (+edit-visibility, +update-ownership, +upload) y `/api/upload` (XHR valums: fichero en body + `qqfile` en query, auto-rename `_N`, sin validación de extensiones server 1:1) + `/file_manager/download` — tests `tests/file-manager.test.ts` (13) + `tests/e2e/file-manager.e2e.test.ts` (1 flujo completo)
- [x] pics (Ctrl_pic2db + bol_bible_refs.picture + resources.3bmoodle.dk): script CLI `bun run pic2db` (port 1:1 de Ctrl_pic2db: fetch jsonrefs.php/jsonurls.php, drop+recreate bol_bible_refs/bol_bible_urls e insert con book_number_from_picdb + array_unique) en `scripts/pic2db.ts` + `src/lib/services/pic2db.ts` — assets `public/images/` (p.png, pblue.png, u/v/d.png para los iconos del texto); el resto ya portado (Picdb → `src/lib/corpus/picdb.ts`, render `?icons=on` en displaymonadobject.ts, gestor /urls) — tests `tests/pic2db.test.ts` (3)
- [ ] SHEBANQ (Ctrl_shebanq)
  ...
  - [x] `index()` — el legacy no tiene vista: `Ctrl_shebanq::index()` es solo un alias de `import_shebanq()`, ya cubierto por `/shebanq`
